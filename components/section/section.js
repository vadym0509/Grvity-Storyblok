function getSection(block, isShowShadow, indicatorNum){
    var content = `${indicatorNum ? `<div id="section-indicator-container-${indicatorNum}" class="relative w-fit top-[15rem] z-[-1] font-bold text-[7.5rem] md:text-[12.5rem] leading-[6.25rem] md:leading-[11.25rem] text-[#5B7FFF]" style="box-shadow:black 80px 140px 70px -70px, black 0px -35px 40px -50px inset">${indicatorNum}</div>` : ``}
                   <div class="${indicatorNum ? 'mt-16' : 'mt-24'} ${block.full_width === 'boxed' ? 'md:inline-flex' : ''} ">
                        <div class="w-full bg-white ${block.full_width === 'boxed' ? 'lg:max-w-[31.25rem]' : 'lg:max-w-[43.25rem]'} mr-8">
                            <div class="font-bold text-[2.375rem] leading-tight text-[#2B2B2B]">${block.blocks[0].headline}</div>
                            ...
                            <div id="first-section-labels-container" class="${block.blocks[2].labels.length > 2 ? 'lg:inline-flex sm:inline-flex md:block' : 'sm:inline-flex'} mt-6 ">
                                <div class="mr-[0.3125rem] text-[#323B4B]">${block.blocks[2].title}</div>
