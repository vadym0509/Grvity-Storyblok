function getCard(block){
    var content = `<div class="mt-12 w-full py-6 px-6 rounded-[20px] bg-white drop-shadow-card">
            ...
            if(block.detail_url.url) content += `<div class="mt-2 md:mt-4 flex w-full justify-end"><a href="${block.detail_url.url}" class="font-semibold text-[0.9375rem] text-[#5B7FFF] hover:underline">See details</a></div>`            
            content += `</div>`;    
        content += `</div>`;
    return content;
}