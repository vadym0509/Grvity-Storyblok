function getCard(block){
    var content = `<div class="mt-12 w-full h-[25rem] md:h-[19.125rem] py-[1rem] px-6 rounded-[20px] bg-white" style="box-shadow: 4px 4px 40px rgba(138, 148, 166, 0.25);">
            ...
            if(block.detail_url.url) content += `<div class="mt-0 md:mt-4 flex w-full justify-end"><a href="${block.detail_url.url}" class="font-semibold text-[0.9375rem] text-[#5B7FFF] hover:underline">See details</a></div>`            
            content += `</div>`;    
        content += `</div>`;
    return content;
}